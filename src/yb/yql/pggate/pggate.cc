...
Status PgApiImpl::InitRandomState(PgStatement *handle, double rstate_w, uint64_t rand_state_s0, uint64_t rand_state_s1) {
  if (!PgStatement::IsValidStmt(handle, StmtOp::STMT_SAMPLE)) {
    // Invalid handle.
    return STATUS(InvalidArgument, "Invalid statement handle");
  }
  RETURN_NOT_OK(down_cast<PgSample*>(handle)->InitRandomState(rstate_w, rand_state_s0, rand_state_s1));
  return Status::OK();
}
