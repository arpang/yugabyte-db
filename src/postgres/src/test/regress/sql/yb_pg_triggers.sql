...
select * from tttest order by price_id;
delete from tttest where price_id = 2;
select * from tttest order by price_id;
-- what do we see ?

-- get current prices
select * from tttest where price_off = 999999 order by price_id;

-- change price for price_id == 3
update tttest set price_val = 30 where price_id = 3 and price_on = 30;
select * from tttest order by price_id, price_val;

-- now we want to change price_id in ALL tuples
...
--  3. expect 1 after_upd_stmt notice (once per statement).
UPDATE main_table SET a = 50, b = 60 WHERE a != 50;
-- Check end state should be the same as start as trigger_func returns null cancelling row ops.
SELECT * FROM main_table ORDER BY a, b;
...
-- TODO Skip this test in YB as DDLs are not yet transactional #1383.
/*
...
-- In YB adding primary key to ensure f1 order is stable (ascending).
CREATE TABLE trigger_test (h int, f1 int, f2 text, f3 text, primary key (h HASH, f1 ASC));
...
INSERT INTO trigger_test VALUES(1, 1, 'foo', 'bar');
INSERT INTO trigger_test VALUES(1, 2, 'baz', 'quux');
...
select * from upsert order by key;
