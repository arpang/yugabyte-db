...
class CDCSDKYsqlTest : public CDCSDKTestBase {
 public:
  ...
  Status PopulateTablegroupData(Cluster* cluster, int insert_count) {
    auto conn = VERIFY_RESULT(cluster->ConnectToDB(kNamespaceName));
    RETURN_NOT_OK(conn.ExecuteFormat("CREATE TABLEGROUP tg1"));
    // RETURN_NOT_OK(conn.ExecuteFormat(
    //     "CREATE TABLE public.test1(key int primary key, val int) SPLIT INTO 1 TABLETS;"));
    ...
    // conn = VERIFY_RESULT(cluster->ConnectToDB(kNamespaceName));
    // RETURN_NOT_OK(conn.Execute("BEGIN"));
    ...
    // RETURN_NOT_OK(conn.Execute("COMMIT"));
    // RETURN_NOT_OK(conn.ExecuteFormat("INSERT INTO test1 VALUES (1), (2), (3);"));
    // RETURN_NOT_OK(conn.ExecuteFormat("INSERT INTO test2 VALUES (1), (2), (3);"));
    return Status::OK();
  }
...
TEST_F(CDCSDKYsqlTest, YB_DISABLE_TEST_IN_TSAN(TestTablegroup)) {
  ...
  // ASSERT_RESULT(GetTableId(&test_cluster_, kNamespaceName, "test1"));
  CDCStreamId stream_id = ASSERT_RESULT(CreateDBStream(IMPLICIT));
